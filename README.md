...
- **v2.1.6** — Licensing system: Pis must be authorised by an admin before running; RSA-signed 30-day JWT tokens issued by community server; Fleet tab: each device shows Pending/Authorised/Revoked badge with Authorise/Revoke button; revocation takes effect immediately via WebSocket push if the Pi is online; 7-day grace period past token expiry to survive server outages; new Pis get a 24-hour window before lockout; lockout mode stops all routes and shows a "Not Authorised" page; touchscreen shows a red lockout screen when not authorised
